...
def sampleid_startup(chrm=None, genofile_type=None, data_format=None, sampleid_path=0, geno_path=0, geneexp_path=0, ped_path=0, pedinfo_path=0, **kwargs):
	...
	# return_lst = [sampleID, len(sampleID)]
...
def weight_file_info2(w_path, chrm, target, weight_threshold=0, add_cols=[], drop_cols=['ID'], **kwargs):
	info_dict = weight_cols_dtype(get_header(w_path+'_header.txt.gz', zipped=True), add_cols, drop_cols)
	return {'path_pre': w_path, 
		'chrm': chrm, 
		'sampleID': [], 
		'target_ind': info_dict['file_cols'].index('TargetID'), 
		'ES_ind': info_dict['file_cols'].index('ES'), 
...
# read in annotation file or gene expression file; FOR TYPE1 ERROR FILE ONLY
def read_gene_annot_exp2(chrm=None, geneexp_path=None, annot_path=None, cols=['CHROM','GeneStart','GeneEnd','TargetID','GeneName'], col_inds=[0,1,2,3,4], dtype={'CHROM':object,'GeneStart':np.int64,'GeneEnd':np.int64,'TargetID':object,'GeneName':object}, **kwargs):
	...
	Gene_annot = pd.read_csv(
		annot_path,
		sep='\t',
		header=None)[0].drop_duplicates().astype('str')
...
def read_tabix(start, end, sampleID, chrm, file_cols, col_inds, cols, dtype, path=None, path_pre=None, genofile_type=None, data_format=None, target_ind=5, target=None, weight_threshold=0, raise_error=True, do_zgrep=False, **kwargs):

	if (not path) and path_pre:
		# try:
		path = path_pre + '_' + target + '.txt.gz'
		# command_str = ''.join(['zcat ', path, ' | tail -n+2'])
		try:
			df = pd.read_csv(
				path,
				sep='\t',
		...
		if not do_zgrep:
			...
			command_str = ''.join(['zgrep -e "[[:space:]]', target, '[[:space:]]" ', path])
		...
		elif (genofile_type == 'weight') and (not do_zgrep):
...
def tabix_query_files(start, end, chrm, geno_path=None, gwas_path=None, w_path=None, z_path=None, w_paths=None, **kwargs):
	...
	target_ks = [i for i in range(len(w_paths))]
	empty_target_ks = []
		
	return target_ks, empty_target_ks


def sort_tabix_output(script_dir, out_dir, temp_path, out_path):
	try:
		call_args = [
			get_abs_path(script_dir) + '/sort_tabix_output.sh', 
			temp_path, 
			out_path]

		subprocess.check_call(
			call_args,
			cwd=get_abs_path(out_dir),
...
def weight_k_files_info2(w_paths, chrm, weight_threshold=0, maf_diff=0, add_cols=[], drop_cols=['ID'], **kwargs):
	if maf_diff:
		file_cols = {k:get_header(w_paths[k]+'_header.txt.gz', rename={'ES':'ES'+str(k),'MAF':'MAF'+str(k)}, zipped=True) for k in range(len(w_paths))}
		return {k: {'path_pre': w_paths[k],
			'chrm': chrm,
			'sampleID': [], 
			'target_ind': file_cols[k].index('TargetID'), 
			'data_format': 'weight', 
		...
		file_cols = {k:get_header(w_paths[k]+'_header.txt.gz', rename={'ES':'ES'+str(k)}, zipped=True) for k in range(len(w_paths))}
		return {k: {'path_pre': w_paths[k],
			'chrm': chrm,
			'sampleID': [], 
			'target_ind': file_cols[k].index('TargetID'), 
...
def get_snpIDs(df: pd.DataFrame, flip=False):
	chrms = df['CHROM'].astype('str').str.strip().values
	pos = df['POS'].astype('str').str.strip().values
	ref = df['REF'].str.strip().values
	alt = df['ALT'].str.strip().values
	if flip:
