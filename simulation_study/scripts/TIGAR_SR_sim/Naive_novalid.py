...
parser.add_argument('--gene_exp', type=str, dest='annot_path')
...
if args.sub_dir:
	out_sub_dir = os.path.join(args.out_dir, 'Naive_CHR' + args.chrm)
else:
...
'''********************************
Input Arguments:
Gene Annotation and Expression file: {annot_path}
Chromosome: {chrm}
...
# sampleID, sample_size, exp_info = tg.sampleid_startup(**args.__dict__)

# Read in gene expression info
print('Reading gene expression data.\n')
# GeneExp, TargetID, n_targets = tg.read_gene_annot_exp(**exp_info)
GeneExp, TargetID, n_targets = tg.read_gene_annot_exp(**args.__dict__)
...
info_wk_dtypes = {W(k)+'_N_SNP':np.int64 for k in range(Kweights)}
info_wk_cols = list(info_wk_dtypes.keys())
...
# out_naive_info_cols = ['CHROM', 'GeneStart', 'GeneEnd', 'TargetID', 'GeneName','N_SNP'] + info_wk_cols
out_naive_info_cols = ['CHROM', 'GeneStart', 'GeneEnd', 'TargetID', 'GeneName','N_SNP']
...
def thread_process(num):
	...
	for k in tabix_target_ks:
		...
		while not W_k.empty:
			...
			if Weights.empty:
				W_k = W_k.drop(columns=['TargetID'])
				Weights = W_k
			else:
				W_k['snpIDflip'] = tg.get_snpIDs(W_k, flip=True)

				# new snpIDs in W_k
				W_k1 = W_k[np.logical_not(np.any(W_k[['snpID','snpIDflip']].isin(Weights.snpID.values), axis=1))].reset_index(drop=True)

				# snpIDs in W_k matched with Weights
				W_k2 = W_k[np.any(W_k[['snpID','snpIDflip']].isin(Weights.snpID.values), axis=1)].reset_index(drop=True)

				# if not in Geno.snpIDs, assumed flipped; if flipped, 1 - MAF and -ES
				flip = np.where(W_k2.snpID.isin(Weights.snpID), True, False)

				if not np.all(flip):
					# set correct snpID, MAF, ES
					W_k2['snpID'] = np.where(flip, W_k2.snpID, W_k2.snpIDflip)
					# if args.maf_diff:
					# 	W_k2[MAF(k)] = np.where(flip, W_k2[MAF(k)], 1 - W_k2[MAF(k)])
					# W_k2[ES(k)] = np.where(flip, W_k2[ES(k)], -W_k2[ES(k)])

				# W_k = W_k.drop(columns=['CHROM','POS','REF','ALT','TargetID','snpIDflip'])
				W_k1 = W_k1.drop(columns=['TargetID','snpIDflip'])
				W_k2 = W_k2.drop(columns=['TargetID','snpIDflip'])

				W_k = pd.concat([W_k1, W_k2]).reset_index(drop=True)
	...
	Naive_Weight = Train[['CHROM','POS','REF','ALT']].copy()
	...
	# Naive_Info = Naive_Info.astype(info_naive_dtypes)[out_naive_info_cols]
	Naive_Info = Naive_Info[out_naive_info_cols]
	Naive_Info.to_csv(
		out_naive_info_path,
		sep='\t',
		header=None,
